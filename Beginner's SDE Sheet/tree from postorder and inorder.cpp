//{ Driver Code Starts
/* program to construct tree using inorder and postorder traversals */
#include <bits/stdc++.h>
using namespace std;

/* A binary tree node has data, pointer to left child
   and a pointer to right child */
struct Node {
    int data;
    struct Node* left;
    struct Node* right;

    Node(int x) {
        data = x;
        left = right = NULL;
    }
};

/* This funtcion is here just to test buildTreeUtil() */
void preOrder(Node* node) {
    if (node == NULL) return;

    /* then print the data of node */
    printf("%d ", node->data);

    /* first recur on left child */
    preOrder(node->left);

    /* now recur on right child */
    preOrder(node->right);
}

Node* buildTree(int in[], int post[], int n);

int main() {
    int t, n;
    cin >> t;
    while (t--) {
        cin >> n;
        int in[n], post[n];
        for (int i = 0; i < n; i++) cin >> in[i];
        for (int i = 0; i < n; i++) cin >> post[i];
        Node* root = buildTree(in, post, n);
        preOrder(root);
        cout << endl;
    }
    return 0;
}

// } Driver Code Ends


/* Tree node structure

struct Node
{
    int data;
    struct Node* left;
    struct Node* right;

    Node(int x){
        data = x;
        left = right = NULL;
    }
};*/

//Function to return a tree created from postorder and inoreder traversals.
int findPosition(int in[], int ele, int inOrdStIndex, int inOrdEndIndex, int n) {
        for(int i=inOrdStIndex; i<=inOrdEndIndex; i++) {
            if(in[i]==ele)
                return i;
        }
        return -1;
    }
    Node* solve(int in[], int post[], int &postOrderIndex, int inOrdStIndex, int inOrdEndIndex, int n) {
        
        if(postOrderIndex < 0 || inOrdStIndex>inOrdEndIndex) {
            return NULL;
        }
        
        int ele = post[postOrderIndex--];
        Node* temp = new Node(ele);
        int posi = findPosition(in, ele, inOrdStIndex, inOrdEndIndex, n);
        
        temp->right = solve(in, post, postOrderIndex, posi+1, inOrdEndIndex, n);
        temp->left = solve(in, post, postOrderIndex, inOrdStIndex, posi-1, n);
        
        
        return temp;
    }
    Node* buildTree(int in[],int post[], int n)
    {
        // Code here
        int postOrderIndex = n-1;
        Node* ans = solve(in, post, postOrderIndex, 0, n-1, n);
        return ans;
    }
